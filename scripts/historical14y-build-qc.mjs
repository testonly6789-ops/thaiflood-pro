...
if (national.totalRecurringDistricts !== 881) throw new Error(`Historical 14-year recurring total mismatch: ${national.totalRecurringDistricts}`);

// Vercel only bundles files that are part of a route dependency graph. Writing a
// loose JSON file during postinstall does not make it a public static asset.
// Generate an ES module instead and import it from api/spatial-index.js so the
// verified snapshot is bundled inside the serverless function.
const snapshotPath = new URL('../lib/historical14y-snapshot.generated.js', import.meta.url);
const snapshotText = `// Generated after Historical 14-year QC passed. Do not edit by hand.\nexport default ${JSON.stringify(national)};\n`;
await writeFile(snapshotPath, snapshotText, 'utf8');
console.log(`[historical14y-qc] bundled-snapshot=${snapshotText.length} bytes`);
