...
async function queryGistdaDistrict(province, district) {
  ...
  while (pages < 8) {
    const data = await postForm(FLOOD_URL, {
      where:`PV_TN='${escapeSql(floodProvince)}'`,
...
export async function probeGistdaSpatial(province) {
  ...
  const [provinceCount, anyProvinceSample, spatialAnyCount, spatialProvinceCount, envelopeAnyCount, envelopeProvinceCount] = await Promise.all([
    ...
    postForm(FLOOD_URL,{where:`PV_TN='${escapeSql(floodProvince)}'`,...commonPolygon}),
    postForm(FLOOD_URL,{where:'1=1',...commonEnvelope}),
    postForm(FLOOD_URL,{where:`PV_TN='${escapeSql(floodProvince)}'`,...commonEnvelope}),
  ]);
  return {
    ok:true,
    province,
