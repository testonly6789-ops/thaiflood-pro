...
function normalizeDistrict(value='') {
  return String(value ?? '').trim().replace(/^อำเภอ/,'').replace(/^อ\./,'').replace(/\s+/g,'');
}
...
function districtKey(provinceCode,districtCode,provinceName,districtName) {
  ...
  if (pc && dc) return `${pc}|${dc}`;
  return `${normalizeProvince(provinceName)}|${normalizeDistrict(districtName)}`;
}
...
function buildNationalPayload(merged,gistdaInfo,ddpmInfo,started) {
  ...
  const allDistricts=provinces.flatMap(p=>p.districts.map(d=>({province:p.province,...d})));
  ...
  const qc={
    ...
    passed:provinces.length===77 && COVERAGE_CE.length===14 && unmatched.length===0 && ddpmInfo.provinceCount>=76,
  };
  ...
  return {
    ...
    methodology:{
      gistda:'พ.ศ. 2554–2559: พื้นที่น้ำท่วมซ้ำซาก GISTDA ปี 2005–2016 ใช้ธงรายปีและรหัสอำเภอโดยตรง',
...
// Kept only for backwards compatibility with the temporary diagnostic route.
