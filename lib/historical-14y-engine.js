...
const FLOOD_LAYER_URL = 'https://gistdaportal.gistda.or.th/data/rest/services/FL_Flood/flood_freq11_20/MapServer/0';
const FLOOD_URL = `${FLOOD_LAYER_URL}/query`;
...
function gistdaProvinceName(province='') { return String(province).startsWith('จ.') ? String(province) : `จ.${province}`; }

function bboxFromGeometry(geometry) {
  let xmin=Infinity,ymin=Infinity,xmax=-Infinity,ymax=-Infinity;
  for (const ring of geometry?.rings || []) for (const point of ring || []) {
  ...
  return {xmin,ymin,xmax,ymax,spatialReference:{wkid:3857}};
}
...
async function getJson(url, timeoutMs=18000) {
  ...
  try {
    const response = await fetch(`${url}${url.includes('?') ? '&' : '?'}f=json`, {signal:controller.signal,headers:{'user-agent':'ThaiFlood-Pro/1.0'}});
    if (!response.ok) throw new Error(`GISTDA HTTP ${response.status}`);
    const data = await response.json();
    if (data?.error) throw new Error(data.error.message || 'GISTDA ArcGIS metadata error');
    return data;
...
async function getDistrictBoundaries(province) {
  const data = await postForm(AMPHOE_URL, {
    ...
    outFields:'AP_CODE,AP_TN,PV_CODE,PV_TN',
    returnGeometry:'true',
    outSR:'3857',
  ...
  return (data.features || []).filter(f => f?.geometry?.rings?.length).map(f => ({
    ...
    provinceCode:String(f.attributes?.PV_CODE || '').trim(),
...
async function queryGistdaDistrict(province, district) {
  ...
  const floodProvince = gistdaProvinceName(province);
  const provinceClause = district.provinceCode ? `PV_CODE='${escapeSql(district.provinceCode)}'` : `PV_TN='${escapeSql(floodProvince)}'`;
  while (pages < 8) {
    const data = await postForm(FLOOD_URL, {
      where:provinceClause,
...
export async function probeGistdaSpatial(province) {
  ...
  const envelope = bboxFromGeometry(first.geometry);
  const floodProvince = gistdaProvinceName(province);
  const provinceCode = first.provinceCode;
  const commonPolygon={geometry:first.geometry,geometryType:'esriGeometryPolygon',inSR:'3857',spatialRel:'esriSpatialRelIntersects',returnCountOnly:'true',f:'json'};
  const commonEnvelope={geometry:envelope,geometryType:'esriGeometryEnvelope',inSR:'3857',spatialRel:'esriSpatialRelIntersects',returnCountOnly:'true',f:'json'};
  const codeClause = provinceCode ? `PV_CODE='${escapeSql(provinceCode)}'` : '1=0';
  const [layerMeta, provinceCount, provinceCodeCount, provinceCodeSample, anyProvinceSample, spatialAnyCount, spatialProvinceCount, envelopeAnyCount, envelopeProvinceCount] = await Promise.all([
    getJson(FLOOD_LAYER_URL),
    postForm(FLOOD_URL,{where:`PV_TN='${escapeSql(floodProvince)}'`,returnCountOnly:'true',f:'json'}),
    postForm(FLOOD_URL,{where:codeClause,returnCountOnly:'true',f:'json'}),
    postForm(FLOOD_URL,{where:codeClause,outFields:'PV_TN,PV_CODE,Freq,Y_2020',returnGeometry:'true',outSR:'3857',resultRecordCount:'3',f:'json'}),
    postForm(FLOOD_URL,{where:'1=1',outFields:'PV_TN,PV_CODE,Freq',returnGeometry:'false',resultRecordCount:'10',f:'json'}),
    postForm(FLOOD_URL,{where:'1=1',...commonPolygon}),
    postForm(FLOOD_URL,{where:codeClause,...commonPolygon}),
    postForm(FLOOD_URL,{where:'1=1',...commonEnvelope}),
    postForm(FLOOD_URL,{where:codeClause,...commonEnvelope}),
  ]);
  const sampleFeature = provinceCodeSample.features?.[0] || null;
  return {
    ok:true,
    province,
    floodProvince,
    ...
    firstDistrict:{district:first.district,code:first.code,ringCount:first.geometry.rings.length,firstPoint:first.geometry.rings?.[0]?.[0] || null,envelope},
    layer:{geometryType:layerMeta.geometryType || null,extent:layerMeta.extent || null,spatialReference:layerMeta.extent?.spatialReference || null,fields:(layerMeta.fields || []).map(f=>f.name)},
    floodProvinceCount:Number(provinceCount.count || 0),
    floodProvinceCodeCount:Number(provinceCodeCount.count || 0),
    provinceCodeSample:(provinceCodeSample.features || []).map(f=>({attributes:f.attributes,geometryFirstPoint:f.geometry?.rings?.[0]?.[0] || null})),
    spatialAnyCount:Number(spatialAnyCount.count || 0),
    spatialProvinceCount:Number(spatialProvinceCount.count || 0),
    envelopeAnyCount:Number(envelopeAnyCount.count || 0),
    envelopeProvinceCount:Number(envelopeProvinceCount.count || 0),
    sampleProvinceValues:(anyProvinceSample.features || []).map(f=>f.attributes),
    sampleFeatureGeometryType:sampleFeature?.geometry ? (sampleFeature.geometry.rings ? 'rings' : sampleFeature.geometry.paths ? 'paths' : 'other') : null,
  };
}
...
export async function buildHistoricalProvince(province) {
  ...
  return {
    ...
    sources:[
      {name:'GISTDA flood recurrence 2011–2020',url:FLOOD_LAYER_URL},
