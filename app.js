...
const CACHE_PREFIX = 'thaiflood:ddpm-fast:v2:';
...
#tabPattern .signal-note{font-size:11px!important;line-height:1.55!important;padding:11px 13px!important}
...
await import('/app-core.js?v=20260826-fast3');

const fmtCompact = (n, d = 0) => n == null ? 'ไม่มีข้อมูล' : Number(n).toLocaleString('th-TH', { maximumFractionDigits:d });
...
function compactDistrictTags(data) {
  ...
  if (!names.length) box.innerHTML = '<span class="empty-text">ไม่พบชื่ออำเภอในข้อมูลที่ดึงได้</span>';
}
...
function compactEventCards(data) {
  ...
  document.querySelectorAll('.event-card').forEach(card => {
    ...
    if (sub) sub.textContent = `${fmtCompact(y.recordCount)} ระเบียน${districtCount ? ` • ${fmtCompact(districtCount)} อำเภอ` : ''}`;

    const kpis = card.querySelectorAll('.event-kpis article');
    if (kpis[0]) kpis[0].innerHTML = `<span>ผลรวมครัวเรือนตามระเบียน ปภ.</span><strong>${fmtCompact(y.households)}</strong><p>${fmtCompact(y.recordCount)} ระเบียน${districtCount ? ` • ${fmtCompact(districtCount)} อำเภอ` : ''}</p>`;
    if (kpis[1]) kpis[1].innerHTML = `<span>พื้นที่เกษตรที่ได้รับผลกระทบ</span><strong>${y.agriRai == null ? 'ไม่มีข้อมูล' : `${fmtCompact(y.agriRai)} ไร่`}</strong><p>${y.totalDamageM == null ? 'ความเสียหาย: ไม่มีข้อมูลในชุดนี้' : `ความเสียหายที่มีข้อมูล ${fmtCompact(y.totalDamageM,1)} ล้านบาท`}</p>`;
    ...
    if (grids[0]) {
      ...
      if (notes[1]) notes[1].innerHTML = `<h5>ขอบเขตข้อมูล</h5><p>${districtCount ? `พบข้อมูลใน ${fmtCompact(districtCount)} อำเภอ` : 'ไม่พบชื่ออำเภอ'} • ${fmtCompact(y.recordCount)} ระเบียน</p>`;
    }
...
function compactInsight(data) {
  ...
  el.textContent = years.length
    ? `พบระเบียนอุทกภัยทางการ ${years.length} ปี ครอบคลุมสูงสุด ${fmtCompact(count)} อำเภอ หน้านี้แสดงเฉพาะข้อมูลที่มีในต้นทาง และซ่อนช่องว่างเพื่อลดความรก`
...
// Warm only the currently selected province, rather than downloading many provinces in the background.
setTimeout(() => {
  const name = document.querySelector('#selectedName')?.textContent?.trim();
  if (name && name !== '—' && !cacheRead(name)) {
    window.fetch(`/api/ddpm?province=${encodeURIComponent(name)}`).then(r=>r.json()).catch(()=>null);
  }
}, 1200);
